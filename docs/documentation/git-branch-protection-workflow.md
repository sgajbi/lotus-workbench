...
repository remains rebase-only, enumerates the first-parent interval from the accepted PR base to
its merged tip, checks that interval against the accepted commit count, and dispatches
...
changed merge methods, an unbounded or count-mismatched interval, or revisions that are not
