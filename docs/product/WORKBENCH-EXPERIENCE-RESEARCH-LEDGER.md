...
1. Put one quiet receipt/recheck control beside the Performance page title in Evidence, where the
   summary/detail pair is the complete visible evidence set. Withhold it in Summary, Analysis, Risk
   Review, and Adviser Brief because those modes own independent queries whose receipts are not part
   of this composite.
...
4. Keep the prior admitted receipt and analytics when an ordinary recheck fails. Announce success
   ...
   including clearing its pending or failed presentation and withholding completion after the
   adviser changes to a different mode through the rail or browser history. A permission denial
   still revokes retained evidence even when its mode-specific completion is obsolete.
...
mode-specific auxiliary-query exclusion, mode-change completion fencing, permission revocation,
and success-only confirmation. A populated optimized-browser scenario proves one summary and one
detail read, no idle reads, the Evidence-only control boundary, accessible status, and rendered
evidence under `docs/evidence/issue-787-performance-receipt-age/`. The visible
