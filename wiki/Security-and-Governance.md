...
| Client authority transition | Responses carry an opaque digest of the admitted principal and scope; a change clears Query and component-owned feature state, authenticated denial clears established authority, and late prior-authority responses are refused | Client authority, provider remount/cache, denial-transition, and transport race tests | Digest evidence is source behavior, not a session certification |
