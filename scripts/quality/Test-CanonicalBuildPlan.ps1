...
'@
  ...
  foreach ($name in @('a','b','c')) {
    ...
    & git -C $path init -q
    & git -C $path -c user.name=Fixture -c user.email=fixture@example.invalid -c commit.gpgsign=false commit --allow-empty -qm fixture
  ...
  foreach ($scenario in @('serial','bounded','failure','expired','interrupted','source-drift','source-during')) {
    ...
    $expectedFailure = $scenario -in @('failure','expired','interrupted','source-drift','source-during')
